...
In this example, the robot is suppoed to go through (2.0, -0.5), (1.0, 3.0), (-2.0, 0.0), (0.0, -2.0) points. Out of these (1.0, 3.0) is unreachable. The LLM would need to generate a plan that moves the robot through the points in order considering the unreachable point

### Plan selection

Uncomment the  line related to `generative_3.xml` in the `config/fabric.yaml` file
